...
Lambda Function
  ...
  │
  └─ FlushAsync()  ──┬──▶  all-logs グループ   [DELIVERY class / $0.25/GB]
                      │       → 全レベルのログ
                      │       → S3 へ自動配信（長期保管）
                      │
                      └──▶  error-logs グループ  [STANDARD class / $0.50/GB]
                              → Error 以上のログ
...
| ロググループ                | Log Group Class | 取り込み単価 | 保持期間         | 用途                                  |
| --------------------------- | --------------- | ------------ | ---------------- | ------------------------------------- |
| `/lambda/{app}/all-logs`    | DELIVERY        | $0.25/GB     | 1 日（AWS 固定） | 全ログを S3 へ配信して長期保管        |
| `/lambda/shared/error-logs` | STANDARD        | $0.50/GB     | 7 日（設定可能） | Lambda 横断のエラー監視・アラーム連携 |

- **DELIVERY class**: 取り込み単価が STANDARD の半額（$0.25/GB）。S3 / Firehose への配信専用。GetLogEvents / FilterLogEvents / Logs Insights は使用不可
- **STANDARD class**: Metric Filter・Subscription Filter・GetLogEvents・Logs Insights に対応（$0.50/GB）
...
graph TB
    ...
    subgraph CloudWatch["CloudWatch Logs"]
        ALG["📋 all-logs<br/>DELIVERY class<br/>$0.25/GB・保持: 1日固定"]
        ELG["📋 error-logs<br/>STANDARD class<br/>$0.50/GB・保持: 7日（設定可）"]
...
| ------------------------------- | ---------------------------------------------------- | ---------------------------------------- | ---------------------- | --------------------------------- |
| **CloudWatch Logs（取り込み）** | `aws_cloudwatch_log_group.all_logs`                  | 従量課金: **$0.25/GB**（DELIVERY class） | 5GB/月（常に無料枠内） | ログ量に依存                      |
| **CloudWatch Logs（取り込み）** | `aws_cloudwatch_log_group.error_logs`                | 従量課金: **$0.50/GB**（STANDARD class） | 5GB/月（常に無料枠内） | ログ量に依存                      |
| **CloudWatch Logs（保管）**     | 同上                                                 | 従量課金: $0.03/GB/月                    | 5GB/月                 | DELIVERY=1日固定、STANDARD=設定値 |
...
> 💡 **コスト最適化のポイント**:
> - **DELIVERY class** を使用することで、取り込みコストを半額（$0.25/GB）に抑え、保管も最小化（1日固定で自動削除）
...
### DELIVERY class の特徴と制約

- 取り込み単価は **$0.25/GB**（STANDARD の半額）
- 保持期間は **1 日間固定**（変更不可）
- `GetLogEvents` / `FilterLogEvents` / CloudWatch Logs Insights は使用不可
- Metric Filter / Subscription Filter は使用不可
- **S3 / Firehose への配信専用** が主な用途
